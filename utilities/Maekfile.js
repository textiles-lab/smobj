...
//building against the "nest libs" package for glm; though if you have system-wide install it should just get picked up and used.
...
const utilities = [
	maek.LINK( [ maek.CPP('knitout-to-smobj.cpp'), ...sm_objs ], 'knitout-to-smobj' ),
	maek.LINK( [ maek.CPP('text-to-smobj.cpp'), ...sm_objs ], 'text-to-smobj' ),
	maek.LINK( [ maek.CPP('smobj-to-yarns.cpp'), ...sm_objs ], 'smobj-to-yarns' ),
	maek.LINK( [ maek.CPP('simplify-yarns.cpp'), ...sm_objs ], 'simplify-yarns' ),
	maek.LINK( [ maek.CPP('yarn-units.cpp'), ...sm_objs ], 'yarn-units' ),
	maek.LINK( [ maek.CPP('yarns-to-bccx.cpp'), ...sm_objs ], 'yarns-to-bccx' ),
	maek.LINK( [ maek.CPP('merge-faces.cpp'), ...sm_objs ], 'merge-faces' ),
	maek.LINK( [ maek.CPP('test-code.cpp'), maek.CPP('hinters.cpp'), ...sm_objs ], 'test-code' ),
];


//set the default target to build all the utilities:
maek.TARGETS = [...utilities];
...
function init_maek() {
	...
	if (maek.OS === 'windows') {
		...
		DEFAULT_OPTIONS.LINK = ['link.exe', '/nologo', '/SUBSYSTEM:CONSOLE', '/DEBUG:FULL', '/INCREMENTAL:NO'];
	} else if (maek.OS === 'linux') {
